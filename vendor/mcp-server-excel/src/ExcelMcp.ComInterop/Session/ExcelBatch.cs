...
internal sealed class ExcelBatch : IExcelBatch, IExcelBatchTeardownState
{
    ...
    private readonly bool _allowMacros; // Low security only when this call will run or edit VBA
    ...
    public ExcelBatch(string[] workbookPaths, ILogger<ExcelBatch>? logger = null, bool show = false, TimeSpan? operationTimeout = null, bool allowMacros = false)
        : this(workbookPaths, logger, show, createNewFile: false, isMacroEnabled: false, allowMacros: allowMacros, operationTimeout: operationTimeout)
    {
    }
    ...
    internal static ExcelBatch CreateNewWorkbook(string filePath, bool isMacroEnabled, ILogger<ExcelBatch>? logger = null, bool show = false, TimeSpan? operationTimeout = null, bool allowMacros = false)
    {
        return new ExcelBatch([filePath], logger, show, createNewFile: true, isMacroEnabled: isMacroEnabled, allowMacros: allowMacros, operationTimeout: operationTimeout);
    }

    /// <summary>
    /// Private constructor that handles both opening existing files and creating new ones.
    /// </summary>
    private ExcelBatch(string[] workbookPaths, ILogger<ExcelBatch>? logger, bool show, bool createNewFile, bool isMacroEnabled, bool allowMacros = false, TimeSpan? operationTimeout = null)
    {
        ...
        _staThread = new Thread(() =>
        {
            ...
            try
            {
                ...
                // ForceDisable unless this call explicitly runs or edits VBA.
                // Opening an .xlsm for values, formatting, recalculation, or a screenshot must not
                // execute Workbook_Open.
                ((dynamic)(object)tempExcel).AutomationSecurity = _allowMacros ? 1 : 3;
