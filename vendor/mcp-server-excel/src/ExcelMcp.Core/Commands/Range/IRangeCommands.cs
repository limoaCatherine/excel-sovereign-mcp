...
public interface IRangeCommands
{
    ...
    /// <summary>
    /// Reads the dynamic-array spill range of the top-left cell.
    /// Supported is false when this Excel build has no HasSpill property.
    /// </summary>
    [ServiceAction("get-spill")]
    SpillAreaResult GetSpill(IExcelBatch batch, string sheetName, [RequiredParameter] string rangeAddress);
}

/// <summary>Dynamic-array spill extent of a formula anchor.</summary>
public sealed class SpillAreaResult : OperationResult
{
    /// <summary>False when this Excel build has no HasSpill property.</summary>
    public bool Supported { get; set; }

    /// <summary>True when the anchor cell currently spills.</summary>
    public bool HasSpill { get; set; }

    /// <summary>A1-style spill address, without dollar signs when Excel returns them that way.</summary>
